...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 ...
                 done_callback=None, area_callback=None, post_step_callback=None,
    ...
    def step(self, action_n):
        ...
        for i, a in enumerate(self.agents):
            for j in range(30):
                ang = 2 * math.pi * j / 30
                points = np.array([a.state.p_pos[0] + math.cos(ang) * a.size,
                                   a.state.p_pos[1] + math.sin(ang) * a.size])
                self.traj[i * 30 + j].append(copy.deepcopy(points))
            a.last_traj.append(copy.deepcopy(a.state.p_pos))
        # record observation for each agent
        r = self.area_callback()
        # print(f"now explore area is {r}")
        for i, agent in enumerate(self.agents):
            ...
            info_n.append(info)
        # print(f"now explore area is {r},reward_n[0][0] is {reward_n[0][0]}")
    ...
    def _get_reward(self, agent, r):
        if self.reward_callback is None:
            return 0.0
        return self.reward_callback(agent, self.world, r)
    ...
    def render(self, mode='human', close=False):
        ...
        if self.render_geoms is None:
            ...
            for traj in self.traj:
                geom = rendering.make_polyline(traj, 8)
                self.render_geoms.append(geom)
            # 恢复线条长度
            geom = rendering.make_polyline([[0, 0]], 1)
            self.render_geoms.append(geom)
            # for passing_area in self.passing_area:
            #     pass
        ...
        for i in range(len(self.viewers)):
            ...
            for e, entity in enumerate(self.world.entities):
                ...
                if 'agent' in entity.name:
                    self.render_geoms[e].set_color(*entity.color, alpha=0.5)  # 设置颜色
            ...
            # 目前没想到什么别的方法来统计所有已经探索过的区域，只能保存图片，然后统计色素值
